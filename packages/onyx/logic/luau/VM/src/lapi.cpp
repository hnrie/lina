...
{
    L->userdata = (RobloxExtraSpace *)data;
}
...
void wrap(Proto *proto, uintptr_t* userdata)
{
    if (!proto)
        return;
    proto->userdata = userdata;
    proto->flags = 127;
    for (int i = 0; i < proto->sizep; ++i)
    {
        wrap(proto->p[i], userdata);
    }
}

void set_caps(lua_State *ls, uint64_t caps)
{
    Closure *closure = (Closure *)lua_topointer(ls, -1);
    if (closure and closure->l.p)
    {
        static uintptr_t stored_caps;
        stored_caps = caps;
        wrap(closure->l.p, &stored_caps);
    }
}